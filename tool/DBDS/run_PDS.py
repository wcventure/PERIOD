...
def setup_share_memroy():
    IPC_KEY, MAP_SIZE, IPC_CREAT, IPC_EXCL = 1111, sizeof(c_ulonglong) + sizeof(c_uint)*3 + sizeof(c_uint16)*(SHARE_THREAD_NUM + 3*SHARE_ARRAY_SIZE), 512, 1024
...
def get_info_from_share_memory(P=None, A=None, B=None, T=None, kpNumArray=None, kpYieldArray=None, kpLocArray=None, kpOrderArray=None):
    ...
    if kpYieldArray != None:
        memmove(byref(kpYieldArray), ADDR + sizeof(c_ulonglong) + sizeof(c_uint)*3 + sizeof(c_uint16)*(SHARE_THREAD_NUM), sizeof(c_uint16)*(SHARE_ARRAY_SIZE))
    if kpLocArray != None:
        memmove(byref(kpLocArray), ADDR + sizeof(c_ulonglong) + sizeof(c_uint)*3 + sizeof(c_uint16)*(SHARE_THREAD_NUM + SHARE_ARRAY_SIZE), sizeof(c_uint16)*SHARE_ARRAY_SIZE)
    if kpOrderArray != None:
        memmove(byref(kpOrderArray), ADDR + sizeof(c_ulonglong) + sizeof(c_uint)*3 + sizeof(c_uint16)*(SHARE_THREAD_NUM + 2*SHARE_ARRAY_SIZE), sizeof(c_uint16)*SHARE_ARRAY_SIZE)
...
def memmove_pattern_list(PatternList):
    ...
    memmove(ADDR + sizeof(c_ulonglong) + sizeof(c_uint)*3 + sizeof(c_uint16)*SHARE_THREAD_NUM, tmparr, sizeof(c_uint16) * len(tmp))
...
B = c_uint(0)
T = c_uint(0)
kpNumArray = (c_uint16*SHARE_THREAD_NUM)(0)
kpYieldArray = (c_uint16*SHARE_ARRAY_SIZE)(0)
kpLocArray = (c_uint16*SHARE_ARRAY_SIZE)(0)
...
if N > SHARE_THREAD_NUM or M * (N + zero_in_KP) > SHARE_ARRAY_SIZE:
    print("\033[31m", "ERROR: Too many Key Points or Threads.", "\033[0m")
    print("N = ", N, "M = ", M)
    sys.exit(1)
...
if N <= 20:
    ZeroPattern = [[row for col in range(M)] for row in range(N + zero_in_KP)]
    for each in ZeroPattern:
        for i in range(1,len(each)):
            each[i] = 0
else:
    ZeroPattern = [[0]*M]*(N + zero_in_KP)
...
for iterPeriod in range(2, period_limit+1):
    ...
    if VarModeFlag == 0 or iterPeriod == 2:
        
        MList = []
        for i in range(0, N_after_DryRun + zero_in_KP):
            MList.append(kpMaxNumArray_after_DryRun[i])
        try:
            scheduler = SBPMESched(N_after_DryRun + zero_in_KP, MList, iterPeriod)
        except:
        ...
        if directInterList != []:
            ...
            if CurdirectInter != []:
                ...
                try:
                    scheduler = SBPMESched(N + zero_num(useful_kp_num_list, [0]*SHARE_THREAD_NUM, N), useful_kp_num_list, iterPeriod)
                except:
    ...
    print("Targeting bugs that bug depth =", iterPeriod-1, ". Iterate for", iterPeriod, "periods")
    ...
    while True:
        ...
        if scheduler != None:
            ...
            for inter in scheduler:
                ...
                if DBDS_stage == 1 and iterPeriod-1 >= 4 and full_deep_run == 0: # for deep bug, explore interesting prefix is enough
                    break
                ...
                rounds = rounds + 1
                if not prefix == []:
                    if is_redundant_with_prefix(inter, prefix): # we can remove this due to "Mengda Add fix_prefix=True"
                        rounds = rounds - 1
                        continue
                ...
                try:
                    ...
                    if coverFlag == 1 :
                        ...
                        #print("[DEBUG] Interesting --> prefix = ", scheduler.get_prefix(), "hash: ", coverage_hashID)
        ...
        if DBDS_stage == 1:
            if VarModeFlag == 0 or iterPeriod == 2:
                ...
                if directInterList != []:
                    ...
                    print("\033[4;36mExploration of these KP:\033[0m", CurdirectInter[2])
                    try:
                        scheduler = SBPMESched(N, useful_kp_num_list, iterPeriod)
                    except:
        ...
        if DBDS_stage == 2:
            ...
            if (iterPeriod == 2): # bug depth <= 1 no need to use prefix
                DBDS_stage = 1
                break
            ...
            while waited_to_explore:
                ...
                try:
                    scheduler = SBPMESched(tempL[2], tmpMList, iterPeriod, prefix, fix_prefix=True) # Mengda Add
                except:
