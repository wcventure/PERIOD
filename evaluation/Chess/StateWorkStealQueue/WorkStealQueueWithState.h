...
public:
	...
	void Push(int tid, int fname)
	{
		...
		if (stacks.size() <= tid)
			stacks.resize(tid+1);
		stacks.at(tid).push_back(ar);
	}

	void Pop(int tid)
	{
		int size = stacks.at(tid).size();
		assert(size > 0);
		stacks.at(tid).resize(size-1);
	}
	...
	ActivationRecord& Peek(int tid)
	{
		int size = stacks.at(tid).size();
		assert(size > 0);
		return stacks.at(tid).at(size-1);
	}
...
class SpinLock {
  ...
  private:
	...
	public:
		...
		// 0 - lock free
		//
		std::atomic<PVOID> volatile lock;
	...
	public:
      ...
      v.compare_exchange_strong(expected, desired, std::memory_order_seq_cst);
      //return t;
      return expected;
		}
