...
string defv[] = {                 /* DEFAULT PARAMETER VALUES              */
    ...
    "nbody=8",                /* number of particles to generate       */
...
string argv[];
{
   ...
   pthread_t child[NPROC-1];
   pthread_t checkT;
   pthread_create(&checkT, NULL, SlaveStartCheck, NULL);
   /* fire off P processes */
   for(ProcessId = 1; ProcessId < NPROC; ProcessId++) {
      pthread_create(&child[ProcessId-1], NULL, SlaveStart, NULL);
   }

   for (ProcessId = 1; ProcessId < NPROC; ProcessId++) {
      pthread_join(child[ProcessId-1], ((void *)0));
   }
